...
save=$2          # 第二个参数：保存路径前缀
json_path=$3     # 第三个参数：数据 json 路径（逗号分隔）

if [ -z "$gpuid" ] || [ -z "$save" ] || [ -z "$json_path" ]; then
    echo "Usage: $0 <gpu_ids> <save_prefix> <json_paths>"
    echo "Example: $0 6,7 /path/to/save /path/to/data1.json,/path/to/data2.json"
    exit 1
fi
